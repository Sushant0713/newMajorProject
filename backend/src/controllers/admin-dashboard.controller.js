...
export const dashboardStatistics = async (req, res) => {
    try {
        let query = `SELECT COUNT(*) as total FROM clients WHERE status = 'active'`;
        ...
        query = `SELECT COUNT(*) as total FROM employees WHERE status = 'active' AND designation != 'admin'`;
        const totalEmployees = await executeQuery(query);

        query = `SELECT COUNT(*) as total FROM processes WHERE status = 'active'`;
        const totalProcesses = await executeQuery(query);

        query = `SELECT COUNT(*) as total FROM data_types WHERE is_active = 1`;
    ...
    query = `SELECT COUNT(*) as this_month_assignments FROM candidate_assignments 
    ...
    query = `SELECT COUNT(*) as this_month_completed FROM candidate_assignments 
    ...
    query = `SELECT e.employee_id, e.full_name, e.designation,
...
export const monthlyOverallSuccessLogs = async (req, res) => {
    try {
        // 1. Query overall logs (no employee filter)
        let query = `
            SELECT 
            ...
            FROM candidate_assignments ca
            WHERE 
                ca.assigned_at >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 11 MONTH), '%Y-%m-01')
        ...
        for (const m of last12Months) {
            ...
            if (found) {
                m.successRate = found.total > 0
                        ? Number((((found.completely_joined + found.joined_count) / found.total) * 100).toFixed(2)) : 0;
            }
        }
        ...
        console.error("Error while fetching overall monthly success logs", error);
        return res.status(500).send("Error while fetching overall monthly success logs");
    }
};